...
ggmap(berkMap) +
  ...
  ggtitle("BPD Stops by Incident Type, 2015-2016") 
```
...
ggmap(berkMap) +
  ...
  ggtitle("BPD Stop Car Searches by Race 2015-2016") +
...
ggmap(berkMap) +
  ...
  ggtitle("BPD Stops: Emergency Psch Evaluation, by Race 2015-2016") 
...
ggmap(berkMap) +
  ...
  ggtitle("BPD Stops by Gender, 2015-2016") 
```
...
sh <- readShapePoly("C:/Users/Rebecca/Dropbox/School/2016_su_School/stat133/finalproject_angry_ladies/Rebeccas_location_Code2/Census_Tract_Polygons2010/Census_tracts_2010.shp")
plot(sh)
sh@data$ID <- as.numeric(sh@data$ID)

# Read in the demographic data and merge on Neighbourhood Id
demo <- read.csv(file="Census_tracts_2010.csv", header=T)
...
# Map 15
```{r}
j <- merge( b3, demo, by = "group"  )

# Plot
bMap12 <- ggmap(berkMap) +
  geom_polygon(aes(x=long,y=lat, group=group, fill=TotalPop), data=j, color='white') +
