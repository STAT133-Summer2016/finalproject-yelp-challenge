# CleanData_MakePercentageTables.R

# Purpose: Manipulates census and stop data to produce small tables of percentages.
#          Please note that this code has already been run, so it doesn't need to be run again.
#          Look in the clean data folder for the .rds file containing the tables.
#          Please look at the documentation below to understand how to read the tables in from the files.

# how to create the percentage tables for bar charts:
library(sp)
library(ggmap)
library(tidyr)
library(plyr)
